...
RUN apt-get -y update && \
    ...
    echo "${NB_USER} ALL=/etc/init.d/mysql start" >> /etc/sudoers && \
    echo "${NB_USER} ALL=/etc/init.d/mysql stop" >> /etc/sudoers && \
    echo "${NB_USER} ALL=/etc/init.d/mysql restart" >> /etc/sudoers
